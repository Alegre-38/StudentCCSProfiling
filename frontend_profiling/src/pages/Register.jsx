...
function Register() {
  ...
  const handleSubmit = async e => {
    ...
    setLoading(true);
    try {
      await register(form.username, form.password, form.password_confirmation);
      navigate('/');
    } catch (err) {
      ...
      <div style={{width:'100%', maxWidth:'400px', background:'#393E46', borderRadius:'20px', overflow:'hidden', boxShadow:'0 24px 60px rgba(0,0,0,0.4)', position:'relative', zIndex:1}}>
        ...
        <div style={{padding:'2.5rem 2.5rem 2rem'}}>
          <div style={{textAlign:'center', marginBottom:'2rem'}}>
            ...
            <h1 style={{margin:0, fontSize:'1.6em', fontWeight:800, color:'#EEEEEE'}}>Student Registration</h1>
            <p style={{margin:'0.3rem 0 0', fontSize:'0.88em', color:'rgba(238,238,238,0.45)'}}>Create your student account</p>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{display:'flex', flexDirection:'column', gap:'1rem'}}>
            <div>
              ...
              <input type="text" name="username" value={form.username} onChange={handleChange} placeholder="Choose a username" required style={inputStyle}
            ...
            <div>
              <label style={{display:'block', marginBottom:'0.4rem', fontSize:'0.82em', fontWeight:600, color:'rgba(238,238,238,0.65)', letterSpacing:'0.04em'}}>CONFIRM PASSWORD</label>
              <input type="password" name="password_confirmation" value={form.password_confirmation} onChange={handleChange} placeholder="Repeat your password" required style={inputStyle}
                onFocus={e => e.target.style.borderColor='#F97316'}
                onBlur={e => e.target.style.borderColor='rgba(255,255,255,0.1)'} />
            </div>
